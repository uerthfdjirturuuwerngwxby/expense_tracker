...
export default function Profile({ user, onLogout, onUserUpdate }) {
  ...
  async function exportData() {
    try{
      ...
      const csv=['Date,Title,Amount,Category,Type',...exps.map(e=>`${e.date},"${e.title}",${e.amount},${e.category},${e.type}`)].join('\n');
                ...
                <div className="hero-spending-bar">
                  ...
                  {budget>0 && (
                    <>
                      ...
                      <p className="hsb-caption">{budgetUsedPct.toFixed(0)}% of ₹{Number(monthlyBudget).toLocaleString('en-IN')} budget used</p>
                    </>
                  )}
                </div>
              )}
            ...
            <SectionCard title="Monthly Budget" icon="wallet" delay={150} accent="#6366f1">
              ...
              {monthlyBudget&&Number(monthlyBudget)>0&&(
                <div className="budget-hint-box">
                  ...
                  <span>Daily limit: <strong>₹{(Number(monthlyBudget)/30).toFixed(0)}</strong> · Weekly: <strong>₹{(Number(monthlyBudget)/4).toFixed(0)}</strong></span>
                </div>
              )}
              ...
              {monthlyBudget&&Number(monthlyBudget)>0&&!expLoading&&(
                <div style={{marginTop:'0.8rem'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                    <span style={{fontSize:'0.76rem',fontWeight:600,color:'#64748b'}}>Total spent vs limit</span>
                  ...
                  <p style={{fontSize:'0.72rem',color:budgetUsedPct>100?'#dc2626':'#94a3b8',marginTop:5,fontWeight:600}}>
                    {budgetUsedPct>100 ? `⚠️ Over budget by ${fmt(totalExpenses-Number(monthlyBudget))}` : `${fmt(Number(monthlyBudget)-totalExpenses)} remaining`}
                  </p>
                </div>
              )}
            </SectionCard>
            ...
            <SectionCard title="Data & Privacy" icon="shield" delay={300} accent="#dc2626">
              ...
              <div className="data-actions">
                <button className="data-btn data-btn--export" onClick={exportData}>
                  <Icon name="download" size={15}/><span>Export as CSV</span>
                </button>
                {deletePhase===0&&(
                ...
                {deletePhase===1&&(
                  <div className="delete-confirm" style={{animation:'fadeUp 0.2s both'}}>
                    ...
                    <button className="data-btn data-btn--danger data-btn--sm" onClick={()=>{setDeletePhase(2);setTimeout(()=>setDeletePhase(0),3000);}}>Yes, Delete</button>
