...
function buildLocalPredictionPayload({ area, monthlyRows, monthlyTarget, selectedMonth }) {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    forecast.push({ month: `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`, expense: Math.round(rolling) });
  }
...
function rewriteForecastMonths(payload, startMonth) {
  if (!payload || !Array.isArray(payload.forecast) || !startMonth) return payload;
  const [y, m] = startMonth.split("-").map(Number);
  if (!y || !m) return payload;
  const remappedForecast = payload.forecast.map((f, i) => {
    const d = new Date(y, (m - 1) + i, 1);
    return { ...f, month: `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}` };
  });
  const actualLine = (payload.charts?.line || []).filter((l) => l.type !== "predicted");
  return {
    ...payload,
    forecast: remappedForecast,
    charts: {
      ...(payload.charts || {}),
      line: [...actualLine, ...remappedForecast.map((f) => ({ ...f, type: "predicted" }))],
    },
  };
}
...
export default function Analytics({ user, onLogout }) {
    ...
    (async () => {
      try {
        if (isTestUser) {
          ...
          setExpenses(monthlyRowsToExpenses(rows).map(e => ({ ...e, amount: san(e.amount) })));
          return;
        }
  ...
  const catTotals = useMemo(() => {
    ...
    const totals = {};
    Object.keys(MODEL_TO_UI).forEach(k => totals[k] = 0);
    let income = 0;
    expenses.forEach(e => {
      const p = (e.date || "").split("-");
      if (p.length !== 3) return;
      if (parseInt(p[0]) !== cy || parseInt(p[1]) - 1 !== cm) return;
      if (e.type === "income" || e.category === "Income") { income += san(e.amount); return; }
      const modelKey = UI_TO_MODEL[e.category] || e.category;
      if (totals[modelKey] !== undefined) totals[modelKey] += san(e.amount);
      else totals["Other"] = (totals["Other"] || 0) + san(e.amount);
    });
    return { ...totals, Income: income };
  }, [expenses]);
  ...
  const monthlyHistory = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      let total = 0;
      expenses.forEach(e => {
        if (e.type === "income" || e.category === "Income") return;
        const p = (e.date || "").split("-");
        if (parseInt(p[0]) === y && parseInt(p[1]) - 1 === m) total += san(e.amount);
      });
      result.push({ month, label, Total_Expense: total });
    }
    return result;
  }, [expenses]);
  ...
  async function runPrediction() {
    ...
    try {
      if (isTestUser) {
        ...
        const adjustedLocal = rewriteForecastMonths(localPayload, targetPredictionMonth);
        setPredData(adjustedLocal);
        setForecastData(adjustedLocal);
        return;
      }
      ...
      const res = await fetch(`${API}/api/analytics/prediction?${query.toString()}`, {
        credentials: "include",
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || "Prediction request failed");
      const adjustedRemote = rewriteForecastMonths(json, targetPredictionMonth);
      setPredData(adjustedRemote);
      setForecastData(adjustedRemote);
    } catch (e) {
      // Fallback to local estimation from available history
      const localPayload = buildLocalPredictionPayload({
        area,
        monthlyRows: allMonthlyRows,
        monthlyTarget,
      ...
      if (localPayload) {
        const adjustedFallback = rewriteForecastMonths(localPayload, targetPredictionMonth);
        setPredData(adjustedFallback);
        setForecastData(adjustedFallback);
        setPredError("Using local prediction based on available previous months.");
      } else {
        setPredError(e?.message || "Could not fetch prediction from /api/analytics/prediction.");
      }
    } finally {
  ...
  const nextMonthExp = forecast[0]?.expense || predData?.predicted_expense || 0;
  const areaBarEntry = forecastData?.charts?.bar?.find(b => b.area === area);
  const areaAvg = areaBarEntry?.area_average || areaBarEntry?.expense || 0;
  ...
  const budgetDrivenTotal = hasMonthlyBudget ? monthlyTarget : (nextMonthExp || totalMonthly || 0);
  ...
  const predictedByCategory = useMemo(() => {
    ...
    modelKeys.forEach((k) => {
      ...
      out[k] = denominator > 0 ? Math.round((base / denominator) * budgetDrivenTotal) : 0;
    });
    return out;
  }, [recommended, catTotals, budgetDrivenTotal]);

  // Category comparison: actual vs budget-driven predicted
  ...
  }, [catTotals, predictedByCategory]);

  const sparkValues = monthlyHistory.map(m => m.Total_Expense);
  const thisMonth = monthlyHistory[monthlyHistory.length - 1];
  const prevMonth = monthlyHistory[monthlyHistory.length - 2];
  const monthChange = prevMonth?.Total_Expense > 0
    ? ((thisMonth?.Total_Expense - prevMonth?.Total_Expense) / prevMonth?.Total_Expense * 100).toFixed(1)
    : null;
  ...
  const reportLines = [
    {
      label: "Current Month",
      text: `In ${area}, your current month spending is ${fmt(totalMonthly)}, and your next month planned spend is ${fmt(budgetDrivenTotal)} based on ${hasMonthlyBudget ? "your budget" : "current trend"}.`,
    },
    {
      label: "Budget Status",
      text: hasMonthlyBudget
        ? `Your monthly budget is ${fmt(monthlyTarget)}. You are ${budgetGap > 0 ? `${fmt(budgetGap)} over budget` : `${fmt(Math.abs(budgetGap))} under budget`}.`
    ...
    },
    {
      label: "Outcome Plan",
      text: hasMonthlyBudget
        ? `If you follow this category plan, you can stay near ${fmt(monthlyTarget)} and protect savings while keeping essential categories healthy.`
  ...
  const smartInsights = [
    ...
    },
    {
      ...
      detail: hasMonthlyBudget
        ? `Following this plan improves budget discipline and helps keep end-of-month spend close to ${fmt(monthlyTarget)}.`
      ...
      <main className="an-main">
        ...
        <div className="stats-grid">
          <div className="stat-card stat-card--primary">
            ...
            <div className="stat-top">
              <div>
                ...
                <p className="stat-val">{fmt(totalMonthly)}</p>
                {monthChange !== null && (
                  <p className={`stat-change ${Number(monthChange) > 0 ? "stat-change--up" : "stat-change--down"}`}>
                    <Icon name={Number(monthChange) > 0 ? "trending-up" : "trending-down"} size={12} />
            ...
            <p className="stat-meta">{Object.values(catTotals).filter(v => v > 0).length} categories · {expenses.filter(e => {
              const now = new Date(); const p = (e.date || "").split("-");
              return parseInt(p[0]) === now.getFullYear() && parseInt(p[1]) - 1 === now.getMonth();
          ...
          <div className="stat-card">
            ...
            <div className="stat-top">
              <div>
                ...
                <p className="stat-val" style={{ color: budgetDrivenTotal > 0 ? (budgetDrivenTotal > totalMonthly ? "#ea580c" : "#16a34a") : "#94a3b8" }}>
                  {budgetDrivenTotal > 0 ? fmt(budgetDrivenTotal) : "—"}
                </p>
                {budgetDrivenTotal > 0 && totalMonthly > 0 && (
                  <p className={`stat-change ${budgetDrivenTotal > totalMonthly ? "stat-change--up" : "stat-change--down"}`}>
                    <Icon name={budgetDrivenTotal > totalMonthly ? "trending-up" : "trending-down"} size={12} />
                    {Math.abs(((budgetDrivenTotal - totalMonthly) / totalMonthly) * 100).toFixed(1)}% vs current
                  </p>
                )}
              </div>
            ...
            <p className="stat-meta">{forecast.length > 0 ? `${forecast.length}-month AI forecast for ${area}` : "Run prediction to see forecast"}</p>
            <p className="stat-meta">Prediction target: {targetPredictionMonth}</p>
          </div>
          ...
          <div className="stat-card">
            ...
            <div className="stat-top">
              <div>
                <p className="stat-label">Area Average ({area})</p>
                <p className="stat-val" style={{ color: areaAvg > 0 ? "#059669" : "#94a3b8" }}>
                  {areaAvg > 0 ? fmt(areaAvg) : "—"}
                </p>
                {areaAvg > 0 && totalMonthly > 0 && (
                  <p className={`stat-change ${totalMonthly > areaAvg ? "stat-change--up" : "stat-change--down"}`}>
                    <Icon name={totalMonthly > areaAvg ? "trending-up" : "trending-down"} size={12} />
                    You spend {Math.abs(((totalMonthly - areaAvg) / areaAvg) * 100).toFixed(1)}% {totalMonthly > areaAvg ? "more" : "less"}
                  </p>
                )}
              </div>
            ...
            <p className="stat-meta">{areaAvg > 0 ? "Based on local spending data" : "Run prediction to compare"}</p>
          </div>
        </div>
          ...
          <div className="section-body">
            {predData || Object.keys(predictedByCategory).length > 0 ? (
              <div className="comparison-grid">
                <div className="chart-panel">
                  <h3 className="chart-panel-title">Your Spending vs Budget-Based Prediction</h3>
                  <p className="chart-panel-sub">Actual vs predicted category spend for {area} using monthly budget when available</p>
                    ...
                    <div className="compare-sum-list">
                      ...
                      {budgetDrivenTotal > 0 && (
                        <div className="csl-row">
                          ...
                          <span className="csl-val">{fmt(budgetDrivenTotal)}</span>
                        </div>
                      )}
                      {areaAvg > 0 && (
                        <div className="csl-row">
              ...
              <div className="empty-cta-card">
                ...
                <p>Select your area and click <strong>Predict My Expenses</strong> to see a comparison of your spending vs what's typical for your area.</p>
              ...
              <div className="advice-grid">
                <div className="advice-panel">
                  <div className="advice-panel-header">
                    ...
                    <div>
                      ...
                      <p className="chart-panel-sub">Budget-aware and area-aware guidance for your {area} spending patterns</p>
                    </div>
                  </div>
                ...
                {Object.keys(predictedByCategory).length > 0 && (
                  <div className="rec-panel">
                    <h3 className="chart-panel-title" style={{ marginBottom: "1.2rem" }}>Budget-Based Category Plan</h3>
