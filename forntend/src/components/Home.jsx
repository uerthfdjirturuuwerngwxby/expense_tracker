              ...
              {NAV_LINKS.map((link,i)=>(
                <Link key={link.label} to={link.to} ref={el=>linkRefs.current[i]=el}
                  ...
                  <span style={{ display:"inline-flex", alignItems:"center", gap:6 }}>
                    <Icon name={NAV_ICONS[link.label] || "circle"} size={13} />
                    {link.label}
                  </span>
                </Link>
              ))}
...
.pill-bg{
  position:absolute;top:0.3rem;bottom:0.3rem;background:white;border-radius:9999px;
...
.pill-link{
  position:relative;z-index:1;padding:0.4rem 1.1rem;border-radius:9999px;
