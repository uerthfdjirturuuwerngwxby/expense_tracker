...
const fmtD = (n) => 'INR ' + (Number(n)||0).toFixed(2);
const san  = (n) => { const v = Number(n)||0; return (v>0&&v<1) ? Math.round(v*1000*100)/100 : v; };
const safeText = (value) => String(value || '').trim();
const normalizeDateInput = (value) => {
  const text = safeText(value);
  return /^\d{4}-\d{2}-\d{2}$/.test(text) ? text : new Date().toISOString().split('T')[0];
};
...
const CategoryChart = React.memo(function CategoryChart({ categoryData, onTooltip }) {
                ...
                labels: {
                  ...
                  generateLabels: (chart) => {
                    ...
                    return d.labels.map((label, i) => ({
                      text: `${label}  INR ${d.datasets[0].data[i].toFixed(0)}`,
  ...
  if (!categoryData || !categoryData.length) {
    return <div className="chart-empty"><span>N/A</span><p>No expense data yet.<br />Add some expenses to see your spending breakdown!</p></div>;
  }
...
const TrendChart = React.memo(function TrendChart({ trendData, hasExpenses }) {
  ...
  if (!trendData || !hasExpenses) {
    return <div className="chart-empty"><span>N/A</span><p>No data yet.<br />Add expenses to see your spending trends!</p></div>;
  }
  return <canvas ref={canvasRef} style={{ width: '100%', height: '100%' }} />;
});
...
export default function Expense({ user, onLogout }) {
  ...
  const [selectedChartMonth, setSelectedChartMonth] = useState(monthInputValue());
  const [selectedTxMonth, setSelectedTxMonth] = useState(monthInputValue());
  ...
  useEffect(() => {
    ...
    const [selectedChartYearRaw, selectedChartMonthRaw] = selectedChartMonth.split('-').map(Number);
    const selectedChartYear = selectedChartYearRaw || cy;
    const selectedChartMonthIndex = (selectedChartMonthRaw || (cm + 1)) - 1;
    ...
    const buildCategory = () => {
      const filtered = expenses.filter(e => {
        ...
        if (timeRange === 'custom_month') return dp.y === selectedChartYear && dp.m === selectedChartMonthIndex;
        return true;
      });
      const cats = {};
      filtered.forEach(e => {
  ...
  const isEntryInRange = (entry, range) => {
    ...
    const [selectedTxYearRaw, selectedTxMonthRaw] = selectedTxMonth.split('-').map(Number);
    const selectedTxYear = selectedTxYearRaw || cy;
    const selectedTxMonthIndex = (selectedTxMonthRaw || (cm + 1)) - 1;
    if (range === 'this_month') return dp.y === cy && dp.m === cm;
    if (range === 'last_month') return dp.y === py && dp.m === pm;
    if (range === 'past_3_months') {
      const ma = (cy - dp.y) * 12 + (cm - dp.m);
      return ma >= 0 && ma < 3;
    }
    if (range === 'custom_month') return dp.y === selectedTxYear && dp.m === selectedTxMonthIndex;
    return true;
  };

  const txRangeLabel = txRange === 'this_month'
    ? 'This Month'
    : txRange === 'last_month'
      ? 'Last Month'
      : txRange === 'past_3_months'
        ? 'Past 3 Months'
        : txRange === 'custom_month'
          ? selectedTxMonth
        : 'All';

  const rangedExpenses = expenses.filter(e => e.type === 'expense' && isEntryInRange(e, txRange));
  const rangedExpenseTotal = rangedExpenses.reduce((sum, e) => sum + san(e.amount), 0);
  const rangedTransactionCount = rangedExpenses.length;
  const selectedOverviewStats = React.useMemo(() => {
    if (selectedOverviewIdx == null || !overviewData?.catData || !overviewData?.labels?.length) return null;
    const label = overviewData.labels[selectedOverviewIdx];
    if (label == null) return null;

    let total = 0;
    let transactionCount = 0;
    Object.values(overviewData.catData).forEach((series) => {
      const bucket = series?.[selectedOverviewIdx];
      if (!bucket) return;
      total += san(bucket.total);
      transactionCount += bucket.items?.length || 0;
    });
  ...
  async function handleSubmit(ev) {
    ...
    const payload = {
      title: safeText(titleRef.current?.value),
      amount,
      category,
      date: normalizeDateInput(dateRef.current?.value),
    };
    if (!payload.title) { notify('Title is required','error'); return; }
    if (!Number.isFinite(amount) || amount <= 0) { notify('Enter an amount greater than 0','error'); return; }
    try {
      if(editingExp) {
  ...
  async function handleDelete(id) {
    try {
      ...
      setExpenses(p=>p.filter(x=>x.id!==id)); notify('Expense deleted');
  ...
  async function handleDeleteGroup(groupId) {
    try {
      ...
      if(!res.ok) { notify('Delete failed','error'); return; }
      setExpenses(p=>p.filter(x=>x.receiptGroupId!==groupId)); notify('Receipt deleted');
    } catch { notify('Network error','error'); }
  }
  ...
  async function confirmReceiptAdd() {
    ...
    const date = normalizeDateInput(analyzedData.date);
    ...
    try {
      ...
      notify(`Added ${merchant} - INR ${grandTotal.toFixed(2)}`);
  ...
  const buildDisplayList = () => {
    ...
    const inTxRange = (entry) => isEntryInRange(entry, txRange);

    const matchesFilter = (entry) => {
      const q = search.toLowerCase();
      const ms = !search
        || safeText(entry.title).toLowerCase().includes(q)
        || safeText(entry.category).toLowerCase().includes(q)
        || safeText(entry.receiptMerchant).toLowerCase().includes(q);
    ...
    const filteredGroups = Object.values(groups).map(g => ({ ...g, visibleItems: g.items.filter(entry => matchesFilter(entry)) })).filter(g => {
      const itemMatch = g.visibleItems.length > 0;
  ...
  const OverviewChart = () => {
    if (!overviewData) return <div className="chart-empty"><span>N/A</span><p>No data yet</p></div>;
    ...
    if (!activeCats.length) return <div className="chart-empty"><span>N/A</span><p>No expense data for this period</p></div>;
      ...
      <main className="main">
        <div className="page-hd">
          <div>
            ...
            <div className="page-area-chip">
              <Icon name="globe" size={13} />
              <span>Area: {userArea}</span>
            </div>
          </div>
          ...
          <div className="scard scard--expenses">
            ...
            <div className="scard-top">
              <div>
                ...
                <p className="scard-val" style={{color:'#dc2626'}}>{fmt(summaryExpenseTotal)}</p>
              </div>
            ...
            <p className="scard-meta">{summaryTransactionCount} expense transaction{summaryTransactionCount!==1?'s':''}</p>
            <p className="scard-meta" style={{marginTop:2,color:'#ef4444',fontWeight:600}}>{summaryLabel}: {fmt(summaryExpenseTotal)}</p>
          </div>
          ...
          <aside className="sidebar">
            ...
            <div className="sidebar-card" style={{marginTop:'1rem'}}>
              ...
              <div className="qs-list">
                ...
                <div className="qs-row"><span className="qs-lbl">Avg. Expense</span><span className="qs-val">{fmt(summaryTransactionCount>0?summaryExpenseTotal/summaryTransactionCount:0)}</span></div>
                ...
                <div className="qs-row"><span className="qs-lbl">{summaryLabel}</span><span className="qs-val" style={{color:'#ef4444'}}>{fmt(summaryExpenseTotal)}</span></div>
              </div>
            </div>
          </aside>
            ...
            <div className="chart-toolbar">
              ...
              <div className="chart-controls">
                <select className="range-sel" value={timeRange} onChange={e=>{ setTimeRange(e.target.value); setTooltip(t => ({ ...t, visible: false })); setSelectedOverviewIdx(null); }}>
                  ...
                  <option value="custom_month">Choose Month</option>
                  <option value="all">All</option>
                </select>
                {timeRange === 'custom_month' && (
                  <input className="range-sel" type="month" value={selectedChartMonth} onChange={e=>{ setSelectedChartMonth(e.target.value); setTooltip(t => ({ ...t, visible: false })); setSelectedOverviewIdx(null); }} />
                )}
              </div>
            </div>
            ...
            <div className="chart-card">
              ...
              <div className="chart-summary">
                <div className="cs-box" style={{ background: '#fef2f2' }}>
                  ...
                  <p className="cs-val" style={{ color: '#dc2626', fontSize: '1.3rem' }}>{fmt(summaryExpenseTotal)}</p>
                </div>
                <div className="cs-box" style={{ background: '#fff7ed' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                    ...
                    <p className="cs-lbl" style={{ color: '#ea580c' }}>{summaryLabel}</p>
                  </div>
                  <p className="cs-val" style={{ color: '#ea580c', fontSize: '1.3rem' }}>{fmt(summaryExpenseTotal)}</p>
                </div>
                <div className="cs-box" style={{ background: '#eff6ff' }}>
                  ...
                  <p className="cs-val" style={{ color: '#2563eb', fontSize: '1.3rem' }}>{summaryTransactionCount}</p>
                </div>
              </div>
            </div>
            ...
            <div className="tx-card">
              <div className="tx-head">
                ...
                <div className="tx-filters">
                  ...
                  {txRange === 'custom_month' && (
                    <input className="range-sel" type="month" value={selectedTxMonth} onChange={e=>setSelectedTxMonth(e.target.value)} />
                  )}
              ...
              <div className="tx-list">
                ...
                {displayList.map(entry => {
                  if(entry.isGroup) {
                    const g=entry;
                    const visibleItems=g.visibleItems||g.items;
                    ...
                    const itemCount=visibleItems.filter(e=>!e.isTax).length;
                    return (
                      <div key={g.groupId} className="receipt-group">
                        <div className="rg-header" onClick={()=>setExpandedGroups(p=>({...p,[g.groupId]:!isExpanded}))}>
                          <div className="rg-ic"><Icon name="receipt" size={18}/></div>
                          <div className="rg-info">
                            <p className="rg-name">{g.merchant}</p>
                            <p className="rg-meta"><span className="rg-badge">Receipt</span>{joinMeta(`${itemCount} item${itemCount!==1?'s':''}${taxVal>0?' + tax':''}`, dateStr)}</p>
                          </div>
                  ...
                  return (
                    <div key={e.id} className="tx-row">
                      ...
                      <div className="tx-info">
                        ...
                        <p className="tx-meta"><span className="tx-cat-dot" style={{background:cc.color}}/>{joinMeta(e.category, dateStr)}</p>
                      </div>
          ...
          <div className="modal-box">
              ...
              <div className="modal-hd-left">
                ...
                {!editingExp&&(
                  <div className="mode-pills">
                    <button className={`mode-pill${modalMode==='manual'?' mode-pill--on':''}`} onClick={()=>{setModalMode('manual');setReceiptFile(null);setReceiptPreview(null);setAnalyzedData(null);setItemsExpanded(false);setTaxPct('');}}>Manual</button>
              ...
              <div className="receipt-body">
                ...
                {receiptFile&&(
                  <div className="receipt-preview-wrap">
                    ...
                    {!receiptPreview&&<div className="pdf-placeholder"><span>PDF</span><span>{receiptFile.name}</span><button className="receipt-change" onClick={()=>fileInputRef.current?.click()}>Change</button></div>}
                        ...
                        <div className={`receipt-summary-card${itemsExpanded?' receipt-summary-card--open':''}`} onClick={()=>setItemsExpanded(p=>!p)}>
                          <div className="rsc-left">
                            ...
                            <div className="rsc-meta">
                              ...
                              <span className="rsc-date">{joinMeta(normalizeDateInput(analyzedData.date), `${(analyzedData.items||[]).length} items`)}</span>
                            </div>
                          </div>
                          <div className="rsc-right">
                            <div className="rsc-total-wrap">
                              <span className="rsc-total-lbl">{taxRate>0?'Selected Grand Total':'Selected Total'}</span>
                              <span className="rsc-total-amt">INR {grandTotal.toFixed(2)}</span>
                            </div>
                        ...
                        <div className={`items-panel${itemsExpanded?' items-panel--open':''}`}>
                            ...
                            {(analyzedData.items||[]).map((item,i)=>{
                              ...
                              return (
                                <div key={i} className={`item-row${isChecked?' item-row--checked':''}`} onClick={e=>{e.stopPropagation();setSelectedItems(p=>({...p,[i]:!isChecked}));}}>
                                  ...
                                  <span className="item-amt">INR {parseFloat(item.amount||0).toFixed(2)}</span>
                                </div>
                              );
                            })}
                          </div>
                          <div className="tax-row">
                            ...
                            {taxRate>0&&(
                              <div className="tax-breakdown">
                                <span className="tax-bd-row"><span>Subtotal ({selectedCount} items)</span><span>INR {itemsSubtotal.toFixed(2)}</span></span>
                                <span className="tax-bd-row tax-bd-row--tax"><span>Tax ({taxRate}%)</span><span>+INR {taxAmount.toFixed(2)}</span></span>
                              </div>
                            )}
                          </div>
                          <div className="items-grand-total">
                            ...
                            <div className="igt-right">
                              ...
                              <span className="igt-val">INR {grandTotal.toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="modal-footer">
                  ...
                  <button type="button" className="btn-save" disabled={!analyzedData||analyzing||selectedCount===0} onClick={confirmReceiptAdd} style={{opacity:(!analyzedData||analyzing||selectedCount===0)?0.5:1}}>
                    {analyzing?'Analyzing...':`Add ${selectedCount>0?selectedCount+' Item'+(selectedCount>1?'s':''):'Items'} - INR ${grandTotal.toFixed(2)}`}
                  </button>
                </div>
              </div>
            )}
            ...
            {(modalMode==='manual'||editingExp)&&(
              <form onSubmit={handleSubmit} className="modal-form">
                <div className="mf-field"><label>Title</label><input ref={titleRef} required placeholder="e.g. Lunch at Cafe" className="mf-inp"/></div>
                <div className="mf-row">
                  <div className="mf-field"><label>Amount (INR)</label><input ref={amountRef} type="number" step="0.01" min="0" required placeholder="0.00" className="mf-inp"/></div>
...
.page-area-chip{margin-top:0.7rem;display:inline-flex;align-items:center;gap:0.45rem;padding:0.42rem 0.72rem;border-radius:999px;background:#eef2ff;color:#4f46e5;font-size:0.82rem;font-weight:700;border:1px solid #c7d2fe;}
