...
function monthDiff(a, b) {
  const [aYear, aMonth] = String(a || "").split("-").map(Number);
  const [bYear, bMonth] = String(b || "").split("-").map(Number);
  if (!aYear || !aMonth || !bYear || !bMonth) return 0;
  return ((aYear - bYear) * 12) + (aMonth - bMonth);
}

function normalizeMonthlySeries(monthlyRows) {
  ...
  for (const row of monthlyRows) {
    ...
    if (cursor) {
      const gapMonths = ((current.getFullYear() - cursor.getFullYear()) * 12) + (current.getMonth() - cursor.getMonth());
      for (let gap = 1; gap < gapMonths; gap += 1) {
        const missing = new Date(cursor.getFullYear(), cursor.getMonth() + gap, 1);
        const filler = {
          area: row.area,
          month: `${missing.getFullYear()}-${String(missing.getMonth() + 1).padStart(2, "0")}`,
          Income: 0,
          Total_Expense: 0,
        };
        for (const key of MODEL_KEYS) filler[key] = 0;
...
function nextMonthKey(month) {
  ...
  if (!year || !monthNumber) return new Date().toISOString().slice(0, 7);
  const date = new Date(year, monthNumber, 1);
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
}

function normalizePredictionTargetMonth(lastActualMonth, requestedMonth) {
  const fallback = nextMonthKey(lastActualMonth);
  if (!requestedMonth) return fallback;
  return monthDiff(requestedMonth, lastActualMonth) > 0 ? requestedMonth : fallback;
}
...
function buildCategoryForecasts(monthlyRows, totalForecast) {
  const recentRows = monthlyRows.slice(-6);
  const latest = recentRows[recentRows.length - 1] || {};
  const previous = recentRows[recentRows.length - 2] || {};
  const totalRecentExpense = recentRows.reduce((sum, row) => sum + sanitizeNum(row.Total_Expense), 0);
  const result = [];

  for (const forecastRow of totalForecast) {
    const targetTotal = sanitizeNum(forecastRow.expense);
    const raw = {};
    let rawSum = 0;

    for (const key of MODEL_KEYS) {
      const weightedCategory = weightedAverage(recentRows.map((row) => sanitizeNum(row[key])));
      const latestValue = sanitizeNum(latest[key]);
      const previousValue = sanitizeNum(previous[key]);
      const categoryGrowth = previousValue > 0
        ? Math.max(-0.35, Math.min(0.35, (latestValue - previousValue) / previousValue))
        : 0;
      const share = totalRecentExpense > 0
        ? recentRows.reduce((sum, row) => sum + sanitizeNum(row[key]), 0) / totalRecentExpense
        : 0;
      const baseValue = Math.max(weightedCategory, targetTotal * share);
      const projected = Math.max(0, baseValue * (1 + (categoryGrowth * 0.45)));
      raw[key] = projected;
      rawSum += projected;
    }

    const scale = rawSum > 0 ? targetTotal / rawSum : 0;
    const categories = {};
    for (const key of MODEL_KEYS) categories[key] = Math.round((raw[key] || 0) * scale);
    result.push({ month: forecastRow.month, categories });
  }

  return result;
}

function buildForecast(monthlyRows, periods = 3) {
  const normalizedRows = normalizeMonthlySeries(monthlyRows);
  const lastTwelve = normalizedRows.slice(-12);
  const totals = lastTwelve.map((row) => sanitizeNum(row.Total_Expense));
  const latestTotal = totals[totals.length - 1] || 0;
  const lastMonth = normalizedRows[normalizedRows.length - 1]?.month || new Date().toISOString().slice(0, 7);
  ...
  const trendGrowth = Math.max(-0.12, Math.min(0.18, average(changes)));
  const recentAverage = weightedAverage(totals.slice(-6));
  const latestThreeAverage = average(totals.slice(-3));

  for (let i = 1; i <= periods; i += 1) {
    const date = new Date(y, (m - 1) + i, 1);
    const forecastMonth = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;

    const seasonalMatches = lastTwelve
      .filter((row) => monthDiff(forecastMonth, row.month) > 0 && monthDiff(forecastMonth, row.month) % 12 === 0)
      .map((row) => sanitizeNum(row.Total_Expense));
    const seasonalAverage = average(seasonalMatches);

    const trendBase = latestThreeAverage > 0
      ? latestThreeAverage * Math.pow(1 + trendGrowth, i)
      : latestTotal * Math.pow(1 + trendGrowth, i);
    const blended = seasonalAverage > 0
      ? ((trendBase * 0.6) + (seasonalAverage * 0.4))
      : ((trendBase * 0.7) + (recentAverage * 0.3));

    result.push({
      month: forecastMonth,
      expense: Math.max(0, Math.round(blended)),
    });
  }
...
app.get("/api/analytics/prediction", requireAuth, async (req, res) => {
  try {
    ...
    const baseMonth = (req.query.base_month || "ALL").toString();
    const requestedTargetMonth = (req.query.predict_for_month || "").toString();
    ...
    const monthlyRows = buildMonthlyExpenseRows(data || [], area);
    const scopedRows = baseMonth === "ALL"
      ? monthlyRows
      : monthlyRows.filter((row) => row.month <= baseMonth);
    if (!scopedRows.length) return res.status(400).json({ message: "Not enough data for prediction." });

    const latestMonth = scopedRows[scopedRows.length - 1]?.month || "na";
    const predictionTargetMonth = normalizePredictionTargetMonth(latestMonth, requestedTargetMonth);
    const targetDistance = Math.max(1, monthDiff(predictionTargetMonth, latestMonth));
    const forecastPeriods = Math.max(3, targetDistance + 2);
    const forecastStartIndex = Math.max(0, targetDistance - 1);
    const cacheKey = `${req.user.id}:${area}:${baseMonth}:${predictionTargetMonth}:${latestMonth}`;
    const cached = getCached(cacheKey);
    if (cached) return res.json(cached);

    const forecastSeries = buildForecast(scopedRows, forecastPeriods);
    const latest = scopedRows[scopedRows.length - 1];
    const categoryForecastSeries = buildCategoryForecasts(scopedRows, forecastSeries);
    const forecast = forecastSeries.slice(forecastStartIndex, forecastStartIndex + 3);
    const categoryForecastMonthly = categoryForecastSeries.slice(forecastStartIndex, forecastStartIndex + 3);
    const nextCategoryForecast = categoryForecastMonthly[0]?.categories || {};
    const baselineRecommended = buildRecommendedSpending(area, latest);
    const recommended = Object.keys(nextCategoryForecast).length
      ? Object.fromEntries(MODEL_KEYS.map((key) => [
          key,
          Math.round(((sanitizeNum(nextCategoryForecast[key]) * 0.75) + (sanitizeNum(baselineRecommended[key]) * 0.25))),
        ]))
      : baselineRecommended;
    ...
    const categoryActualVsPredicted = MODEL_KEYS.map((key) => ({
      category: key,
      actual: Math.round(sanitizeNum(latest[key])),
      predicted: Math.round(sanitizeNum(nextCategoryForecast[key])),
      delta: Math.round(sanitizeNum(latest[key]) - sanitizeNum(nextCategoryForecast[key])),
    }));

    const payload = {
      area,
      base_month: baseMonth,
      prediction_target_month: predictionTargetMonth,
      forecast,
      advice,
      recommended_spending: recommended,
      category_forecast: nextCategoryForecast,
      category_forecast_monthly: categoryForecastMonthly,
      category_actual_vs_predicted: categoryActualVsPredicted,
      charts: {
        line: buildLineChart(scopedRows, forecast),
        pie,
